...
def _valid_basis(
    ...
    polys = [
        build_tree_polygon(b.x, b.y, b.deg, scale)
        for b in basis
    ]
        ...
        for j, base_b in enumerate(basis):
            for sx in shifts:
                for sy in shifts:
                    ...
                    ty = base_b.y + sy * dy
                    poly_j = build_tree_polygon(tx, ty, base_b.deg, scale)
...
def _random_basis(
    ...
    for _ in range(k):
        for _ in range(attempts):
            ...
            if _valid_basis(candidate, dx, dy, offset, neighbor_range=neighbor_range):
                basis = candidate
                break
        else:
            return None
...
def _refine_candidate(
    ...
    for _ in range(max(1, restarts)):
        ...
        for i in range(steps):
            ...
            if not _valid_basis(nbasis, ndx, ndy, noffset, neighbor_range=args.neighbor_range):
                continue
...
def main() -> None:
    ...
    parser.add_argument("--angle-set", default="0,180")
    parser.add_argument("--angle-jitter", type=float, default=0.0)
    parser.add_argument("--dx-range", default="0.35,0.9")
    parser.add_argument("--dy-range", default="0.45,1.2")
    parser.add_argument("--offset-range", default="0.0,1.0")
    parser.add_argument("--lattice-angle-range", default="0.0,0.0")
    parser.add_argument("--basis-attempts", type=int, default=60)
    ...
    parser.add_argument("--out", default="results/periodic_search.json")
    ...
    for k in k_list:
        ...
        for _ in range(args.trials):
            ...
            solver = PeriodicSolver(cfg)
            proxy_score = _score_subset(solver, score_n_list)
            record = {
                "k": k,
                "dx": dx,
                "dy": dy,
        ...
        for record in best:
            ...
            print(
                f"k={k} proxy={record['proxy_score']:.6f} total={total_score:.6f}",
                flush=True,
            )
