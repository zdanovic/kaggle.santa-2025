...
def score_submission(submission: pd.DataFrame) -> float:
    solution = submission[["id"]].copy()
    return score(solution, submission, "id")


def score_detailed(submission: pd.DataFrame) -> Tuple[float, Dict[int, float]]:
    # remove the leading 's' from submissions
    data_cols = ["x", "y", "deg"]
    submission = submission.astype(str)
    for c in data_cols:
    ...
    for group, df_group in submission.groupby("tree_count_group"):
        ...
        group_score = (Decimal(side_length_scaled) ** 2) / (scale_factor**2) / Decimal(num_trees)
        total_score += group_score
        per_group[int(group)] = float(group_score)

    return float(total_score), per_group
